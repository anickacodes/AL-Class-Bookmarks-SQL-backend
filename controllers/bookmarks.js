...
router.get("/", async (req, resp) => {
  // url/endpoint?<whatsthequery>=<thequery>
  let allBookmarks;
  const { name } = req.query;
  if (name) {
    allBookmarks = await getAllBookmarks(name);
  } else {
...
router.delete("/:bookmarkId", async (req, resp) => {
  ...
  deletedBookmark.id
    ? resp.status(200).json(deletedBookmark)
    : resp.status(404).json({ Error: "Bookmark Not Found" });
});
...
router.put(
  ...
  async (req, resp) => {
    ...
    updatedBookmark.id
      ? resp.status(200).json(updatedBookmark)
      : resp.status(404).json({ Error: "bookmark not found" });
  }
);
